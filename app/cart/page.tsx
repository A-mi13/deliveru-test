...
import DeliveryMethodSelection from "@/components/shared/DeliveryMethodSelection";
import DeliveryAddressesPage from "../profile/address/page";
import PickupPointsMap from "@/components/shared/PickupPointsMap";
...
const CartPage: React.FC = () => {
  ...
  const [isCheckout, setIsCheckout] = useState(false); // Новое состояние для перехода к оформлению
  ...
  const totalItems = Object.values(localCart).reduce((sum, item) => sum + (item.count || 0), 0);
  const totalPrice = calculateTotalPrice(localCart);
  const finalPrice = calculateFinalPrice(totalPrice, deliveryPrice, discount, freeShipping);
  ...
  const handleApplyPromoCode = async () => {
    ...
    try {
      ...
      const { discount, freeShipping, expiresAt, updatedTotalAmount } = await response.json();
      ...
      localStorage.setItem(`promo_${promoCode}`, JSON.stringify({ discount, freeShipping, expiresAt }));
    } catch (err) {
  ...
  const handleCheckout = () => {
    setIsCheckout(true); // Переход к оформлению заказа
  };
  ...
  const handleGoBack = () => {
    setIsCheckout(false); // Возврат к корзине
  ...
  return (
    <div className="p-4 max-w-[430px] mx-auto relative flex flex-col h-screen">
      {isCheckout ? (
        <>
          <button onClick={handleGoBack} className="text-blue-500 text-lg">
            &#x2190; Назад
          </button>
          {!selectedDeliveryMethod ? (
            <DeliveryMethodSelection onSelect={handleDeliveryMethodSelect} />
          ) : (
            <>
              {selectedDeliveryMethod === "pickup" && (
                <div>
                  <h2 className="text-xl font-bold mb-4">Адрес получения</h2>
                  <PickupPointsMap />
                </div>
              )}
              {selectedDeliveryMethod === "courier" && <DeliveryAddressesPage />}
              {selectedDeliveryMethod === "transport" && (
                <div>
                  <h2 className="text-xl font-bold mb-4">Доставка транспортной компанией</h2>
                  {/* Здесь будет компонент для выбора транспортной компании */}
                </div>
              )}
            </>
          )}
        ...
        <>
                  ...
                  <div className="mt-4 flex justify-between items-center">
                    ...
                    <div className="flex items-center bg-[#70B9BE] rounded-full">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          decreaseCount(item.id, item.productItem?.id);
                        }}
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          increaseCount(item.id, item.productItem?.id);
                        }}
                        className="px-3 py-1 text-white"
                      >
                        +
          ...
          <button
            ...
            className="mt-4 w-full bg-[#70B9BE] text-white py-2 rounded-full"
          >
            Оформить заказ
          </button>
        </>
